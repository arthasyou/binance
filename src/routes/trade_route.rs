...
use crate::{
    binance::{account::get_orders, leverage::change_leverage, order::create_order},
    models::{
    ...
    if let Some(mutex) = prices.get(&payload.symbol) {
        ...
        if let Some(mutex_config) = adjustments.get(&payload.adjustment_id) {
            ...
            match order_response {
                Ok(order) => {
                    match get_orders(&payload.symbol, order.orderId).await {
                        Ok(b_order) => {
    ...
    if let Some(mutex) = prices.get(&payload.symbol) {
        ...
        if let Some(mutex_vec) = trades.get(&payload.symbol) {
            ...
            if let Some(index) = trade_list.iter().position(|trade| trade.id == payload.id) {
                ...
                match order_response {
                    Ok(order) => match get_orders(&payload.symbol, order.orderId).await {
                        Ok(b_order) => {
...
async fn get_all_history_trades(
    Extension(database): Extension<DatabaseConnection>,
